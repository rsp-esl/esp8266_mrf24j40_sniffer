...
bool BME280::begin( uint8_t addr, int32_t sda_pin, int32_t scl_pin, int32_t clk_speed ) {
  _i2c_addr = addr;
  _sda_pin  = sda_pin;
  _scl_pin  = scl_pin;
  _speed    = clk_speed;
...
uint8_t BME280::read8( byte reg ) {
  ...
  Wire.write( (uint8_t)reg );
  Wire.endTransmission();
  Wire.requestFrom( (uint8_t)_i2c_addr, (byte)1 );
  value = Wire.read();
  return value;
}
...
uint16_t BME280::read16( byte reg ) {
  ...
  Wire.write( (uint8_t)reg );
  Wire.endTransmission();
  Wire.requestFrom( (uint8_t)_i2c_addr, (byte)2 );
...
uint32_t BME280::read24( byte reg ) {
  ...
  Wire.write( (uint8_t)reg );
  Wire.endTransmission();
  Wire.requestFrom( (uint8_t)_i2c_addr, (byte)3 );
